...
const Historial = () => {
  ...
  // Mostrar número de semana acorde al mes seleccionado (reinicia a 1 por mes)
  const displayedSemanaNumero = React.useMemo(() => {
    if (!semanaData) return null;
    // Si hay filtro de mes/año activo, intentar mostrar el índice dentro del mes
    if (filtroMes && filtroAnio) {
      const mesSeleccionadoData = historicalByMonth.find(m => 
        m.mes === filtroMes && m.anio.toString() === filtroAnio
      );
      if (mesSeleccionadoData && Array.isArray(mesSeleccionadoData.semanas)) {
        const idxEnMes = mesSeleccionadoData.semanas.findIndex(s => s.semana === semanaData.semana);
        if (idxEnMes !== -1) return idxEnMes + 1;
      }
    }
    // Por defecto, mostrar el número global de semana
    return semanaData.semana;
  }, [semanaData, filtroMes, filtroAnio, historicalByMonth]);
          ...
          <h3 className="historial-titulo">
            ...
            {vistaActual === 'semana' && semanaSeleccionada !== null && (
              <div className="titulo-con-navegacion">
                ...
                <span>Semana {displayedSemanaNumero} ({formatearFechaCorta(semanaData.dias[0]?.fecha)} - {formatearFechaCorta(semanaData.dias[semanaData.dias.length - 1]?.fecha)})</span>
                <button 
                  className="nav-button" 
                  onClick={() => navegarSemana(1)}
